...
export default {
  ...
  methods: {
    getStandard(stdId) {
      this.loading = true;
      this.errorHandler(axios.get("/api/evaluation/info/" + stdId))
        .then((response) => {
    ...
    deleteStd() {
      ...
      this.errorHandler(
        axios({
          ...
          data: { id: this.$route.params.stdId },
        })
      )
        .then(() => {
          this.$router.go(-1);
    ...
    submitStd() {
      ...
      var _url = "/api/evaluation/add";
      if (this.$route.params.stdId) {
        _data.id = this.$route.params.stdId;
        _url = "/api/evaluation/update";
      }
      this.errorHandler(
        axios({
          method: "post",
