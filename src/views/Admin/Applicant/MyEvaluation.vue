<template>
  <v-main>
    <v-container fluid>
      ...
      <v-alert v-if="admin_groups.length === 0" text type="error"
        >你不属于任何组别，无法进行评分。</v-alert
      >
      <v-row justify="center" :class="$vuetify.breakpoint.xs ? '' : 'mx-10'">
        <v-col cols="12" md="10" lg="4">
          <v-select
            v-show="admin_groups.length > 1"
            :items="admin_groups"
            v-model="group"
            label="选择评价组别"
            solo
                ...
                >
                  ...
                  <v-list-item-action>
                    <v-rating
                      half-increments
                      :dense="$vuetify.breakpoint.xs"
                      :size="$vuetify.breakpoint.xs ? 28 : 38"
                      background-color="grey lighten-1"
                      color="orange darken-1"
                      v-model="std_score.score"
                      @input="setEvaScore"
                    ></v-rating>
                  </v-list-item-action>
                </v-list-item>
      ...
      <div class="text-center my-3">
        <v-btn
          ...
          @click="deleteScore"
        >
          <v-icon left>mdi-delete</v-icon>删除评分
        </v-btn>
        <v-btn
          class="ma-2"
          color="indigo"
...
export default {
  ...
  mounted() {
    ...
    this.errorHandler(axios.get("/api/scores/" + this.$route.params.appId)).then((response) => {
      if (response.data.score) {
        ...
        if (!response.data.standard_id) this.mounted_loading = false;
      } else {
  ...
  watch: {
    standard_id(new_val) {
      ...
      this.errorHandler(axios.get("/api/evaluation/info/" + new_val))
        .then((response) => {
          this.standard_name = response.data.evaluation_standard.name;
          if (this.mounted_loading) {
            this.mounted_loading = false;
            return;
          }
          var standard = response.data.evaluation_standard.standard;
          if (!standard) return;
  ...
  methods: {
    ...
    submitScore() {
      ...
      this.errorHandler(
        axios({
          method: "post",
          url: "/api/scores/" + this.$route.params.appId,
          data: {
            score: this.score,
            group: this.group,
