<template>
  <v-list-item two-line :to="'/admin/person/' + applicant.id + '/info'">
    <v-list-item-avatar
      :color="applicant.gender == 'male' ? 'blue' : applicant.gender == 'female' ? 'pink' : 'grey'"
    >
    ...
    <v-list-item-content>
      ...
      <v-list-item-subtitle v-if="applicant.time" class="d-inline-block text-truncate"
        >{{ dateTime }}面试</v-list-item-subtitle
      >
      <v-list-item-subtitle v-else class="d-inline-block text-truncate"
        >未选择面试时间</v-list-item-subtitle
      >
    </v-list-item-content>
    <v-list-item-action v-if="applicant.admits && applicant.admits.length">
      ...
      <v-list-item-action-text
        v-if="applicant.admits.length === 1"
        class="title green--text text--darken-3"
        >{{ applicant.admits[0].group }}</v-list-item-action-text
      >
      <v-list-item-action-text v-else class="title green--text text--darken-3"
        >{{ applicant.admits.length }}个组</v-list-item-action-text
      >
    </v-list-item-action>
    <v-list-item-action v-else-if="applicant.scores_count">
      <v-list-item-action-text>本组{{ applicant.scores_count }}人评分</v-list-item-action-text>
      <v-list-item-action-text class="title"
        >{{ (applicant.avg_score / 1).toFixed(1) }} 分</v-list-item-action-text
      >
    </v-list-item-action>
  </v-list-item>
</template>
...
export default {
  ...
  computed: {
    dateTime() {
      return moment(this.applicant.date + "T" + this.applicant.time).format(
        "MMMDoah:mm"
      );
    },
  },
