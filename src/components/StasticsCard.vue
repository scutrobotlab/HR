  ...
  <v-card class="ma-3">
    <v-list-item three-line>
      <v-list-item-content>
        ...
        >
          {{ stastic.group }}
          <v-icon v-if="admitted.includes(this.stastic.group)" color="success"
      ...
      <v-progress-circular
        ...
        rotate="-90"
        :color="
          'hsl(' + rank_p * 120 + ', 100%, ' + ((rank_p - 0.6) * (rank_p - 0.6) * 35 + 20) + '%)'
        "
      >
        排名
        <br />
    ...
    <v-card-actions>
      <span v-if="admin_groups.includes(this.stastic.group)">
        <v-btn
          ...
          v-if="!admitted.includes(this.stastic.group)"
          @click="setAdmit"
          :loading="admit_btn_loading"
          >录用</v-btn
        >
...
export default {
  ...
  mounted() {
    axios
      .get("/api/rank/" + this.stastic.group + "/" + this.$route.params.appId)
      .then((response) => {
